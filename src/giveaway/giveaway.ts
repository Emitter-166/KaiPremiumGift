...
export const giveaway_process = async (serverId: string, sequelize: Sequelize, client: Client): Promise<boolean> => {
    try {
        ...
        if (!await send_thread(threadId, client, "Kai premium winner!", `<@${winner.id}> You just won kai premium 🥳🥳`, winner.id)) return false;
        await send_dm(winner.user, "Congrats!", "" +
            `**Congrats on winning Kai premium! Your code is ||\"${code}\"|| . Please DM <@907606359405633536> \"I want to redeem my gift\" and send her the code :)**`)
        return true;
    } catch (err) {
